...
public final class CraftMenuBindings {
    ...
    private static String forgeVersion() {
        try {
            VersionInfo info = FMLLoader.versionInfo();
            if (info != null && info.forgeVersion() != null) return info.forgeVersion();
    ...
    /** Escala de texto para el binding del logo. */
    public static float textScaleFor(String binding) {
        return isLogoBinding(binding) ? 2.4f : 1f;
    }

    /** Alineación izquierda para líneas de versión (como vanilla). */
